...
if __name__ == '__main__': 
    ...
    X_train, X_test, y_train, y_test = train_test_split(df_train["feature"], df_test["label"], test_size = 0.2, random_state = seed)
    ...
    # df_test = ds.prepare(segments="test", col_set=["feature", "label"], data_key=DataHandlerLP.DK_I)
    # x_train, y_train = df_train["feature"], df_train["label"] # X
    # x_valid, y_valid = df_valid["feature"], df_valid["label"] # y

    """ dtrain = lgb.Dataset(X_train.values, label=y_train)
    dtest = lgb.Dataset(X_test.values, label=y_test)
    ...
    def objective(trial):
        ...
        model, mean_score = cross_validation_fcn(X_train, y_train, model, early_stopping_flag=True)

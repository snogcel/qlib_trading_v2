...
if __name__ == '__main__': 
    ...
    df_all["spread_thresh"] = df_all["spread"].rolling(180).quantile(0.5)
    df_all["signal_thresh"] = df_all["abs_q50"].rolling(180).quantile(0.5)
    ...
    def classify(row):
        if pd.isna(row["spread_thresh"]) or pd.isna(row["signal_thresh"]):
            return "D"  # or "D" for startup grace period
    ...
    # integrate holdout back into df_all
    df_test = pd.concat([
        preds_test["quantile_0.10"].rename("q10"),
        preds_test["quantile_0.50"].rename("q50"),
        preds_test["quantile_0.90"].rename("q90"),
        y_test["LABEL0"].rename("truth"),
        X_test["fg_index"],  # sentiment
        X_test["btc_dom"]    # capital flow
    ], axis=1).dropna()
    
    df_all = pd.concat([df_all, df_test], axis=0, join='outer', ignore_index=False)
    ...
    def objective(trial):
        ...
        w_A = trial.suggest_float("tier_A", 1.00, 1.45)
    ...
    model.learn(total_timesteps=220000, callback=entropy_logger)
